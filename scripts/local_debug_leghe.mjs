...
async function ensureLogged(page){
  const guestBar = page.locator('a:has-text("ACCEDI")').first();
  if (await guestBar.count()){
    await saveDebug(page, 'still_guest');
    throw new Error('Ancora guest (barra ACCEDI visibile). Fai login a mano o controlla credenziali.');
  }
}
